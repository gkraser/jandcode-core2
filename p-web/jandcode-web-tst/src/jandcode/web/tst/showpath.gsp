<%@ page import="jandcode.web.gsp.*; jandcode.web.*; jandcode.commons.*; jandcode.web.tst.*" %>
<jc:page template="page/tst-main">
  <%
    BaseGsp th = this
    String path = th.args.path
    TstRes root = TstRes.createRoot(th.app, path, false, null)
    ...
    def outA0 = { TstRes r, boolean fullpath = false ->
      ...
      if (r.folder) {
        if (UtString.empty(r.path)) {
          rf = th.ref("_tst", [path: "ROOT"])
        } else {
          rf = th.ref("_tst", [path: r.path])
        }
        cls = 'ref-folder'
      } else {
        rf = th.ref("/${r.path}")
