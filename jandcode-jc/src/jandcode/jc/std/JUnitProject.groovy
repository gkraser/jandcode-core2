...
class JUnitProject extends ProjectScript {
    ...
    void runConsoleRunner(CmArgs args) {
        ListLib rlibs = ctx.getLibs([project.name, "junit-platform-console"])

        JavaProject jm = getIncluded(JavaProject)

        jm.recompileClasses()
        ant.mkdir(dir: jm.dirCompiledTest)
        jm.compileTest()

        log "test compile"
        List<String> cp = rlibs.classpath
...
//        println cp_s
        ...
        //def cp=ctx.classpath()

        String ar = ""
        ar += " -cp ${cp_s}"
        ar += " --details=none"
        ar += " --reports-dir=temp/rep"
//        ar += " --select-class=sandbox.junit5.My3_Test"
//        ar += " --select-method=sandbox.junit5.My3_Test#test2"
//        ar += " --select-method=test3"
//        args += " --select-directory=${jm.dirCompiledTest}"
        ar += " --scan-classpath=${jm.dirCompiledTest}"

        String cmd = "java -cp ${cp_s} org.junit.platform.console.ConsoleLauncher ${ar}"
...
//        ut.delim()
        ...
        ant.mkdir(dir: wd("temp/rep.html"))
        ant.junitreport(todir: wd("temp/rep.html")) {
            fileset(dir: wd("temp/rep")) {
                include(name: "**/*.xml")
            }
            report(format: "frames", todir: wd("temp/rep.html"))
        }
